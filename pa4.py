...
def rotate(S, tau):
  ...
  # FIXME: Implement this function.
  # FIXME: Return correct output
  return {complex(z) * cmath.exp(1j * tau) for z in S}
...
# FIXME: Return correct output
  ...
  def __sub__(self, other):
    ...
    overloads the - operator to support Vec - Vec
    :raises: ValueError if vectors are not same length
    :returns: Vec type; a Vec object that is the difference        vector of this Vec
    and 'other' Vec
    """
# FIXME: Finish the implementation
# FIXME: Return correct output
  ...
  def __mul__(self, other):
    ...
    - Vec * Vec (dot product) raises ValueError if vectors are     not same length in the case of dot product; returns scalar
